...
Puppet device modules allow network devices and other targets that cannot run a
puppet agent to be managed from a proxy. To use device modules from apply
statements the devices need to be added to the bolt inventory as remote
targets. The `name` of the target must be the full url to pass to the device
implementation and the "type" value must be set to refer to the device class.
Due to the limitations of the network device framework the `name` must be the
full url network device expects no config options from the `remote` section
